...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`registry_clicks_updated_at_idx\` ON \`registry_clicks\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`registry_clicks_created_at_idx\` ON \`registry_clicks\` (\`created_at\`);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`heroes_id\` integer REFERENCES heroes(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`registry_clicks_id\` integer REFERENCES registry_clicks(id);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_pages_id_idx\` ON \`payload_locked_documents_rels\` (\`pages_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_posts_id_idx\` ON \`payload_locked_documents_rels\` (\`posts_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_events_id_idx\` ON \`payload_locked_documents_rels\` (\`events_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_heroes_id_idx\` ON \`payload_locked_documents_rels\` (\`heroes_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_registry_clicks_id_idx\` ON \`payload_locked_documents_rels\` (\`registry_clicks_id\`);`)
}
